%analyze photometry data of freely moving mouse performing T-maze alternation task, by resampling signal in the maintenance period to the median
%times btween events [T-junction, detection_in_the_start_box, end_of_delay, detection_in_the_corridor]

% authors:Maria Wilhelm and Yaroslav Sych
...
folder_with_data_for_averaging = 'J:\H50_Disk_D\WM_materials_for_the_paper\Data_for_Figures\Data_Figure2\sorted_data\individual_mice_data\GCaMP'; %give the folder with the raw data
...
for current_session = 1:size(sessions_to_average,2)
    ...
    [session_resampled_right_correct_per_session{current_session},dt_resample_R] = ResampledSession_v2(sessions_to_average{current_session}.demodSig488,sessions_to_average{current_session}.Trial_RWhole_Correct(:,[6,7,8,9]),sm_dT_RL_Whole, baseline_end_event_number);
    [session_resampled_left_correct__per_session{current_session},dt_resample_L] = ResampledSession_v2(sessions_to_average{current_session}.demodSig488,sessions_to_average{current_session}.Trial_LWhole_Correct(:,[6,7,8,9]),sm_dT_RL_Whole,baseline_end_event_number);
...
for current_session = 1:size(sessions_to_average,2)-1
    ...
    if any(ismember(fields(sessions_to_average{1, current_session}),'Trial_RWhole_Mistake'))==1
        [session_resampled_right_mistake_per_session{current_session},dt_resample_R_Mistake] = ResampledSession_v2(sessions_to_average{current_session}.demodSig488,...
                                                                                                                    sessions_to_average{current_session}.Trial_RWhole_Mistake(:,[6,7,8,9]),...
                                                                                                                    sm_dT_RL_Whole, baseline_end_event_number);
    end
    if any(ismember(fields(sessions_to_average{1, current_session}),'Trial_LWhole_Mistake'))==1
        [session_resampled_left_mistake_per_session{current_session},dt_resample_L_Mistake] = ResampledSession_v2(sessions_to_average{current_session}.demodSig488,...
...
number_of_draws = 20;
number_of_trials_sampled = 200; %since there are two times more correct trials then mistake, use 100 for Mistakes and 200 for correct Trials comparissons
