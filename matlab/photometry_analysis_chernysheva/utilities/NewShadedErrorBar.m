function varargout=NewShadedErrorBar(x,y,errBar,lineProps,transparent,size,y_axis1,y_axis2,x_axis1,x_axis2,Legend,name,n,Color)
...
%               image. 
% Legend - string name of the graph 'Start' or 'Sound'
% n-number of trials e.g.length
%
% Outputs
...
h_title=title({name;'';'';'';'';'';''});
set(h_title, 'FontSize', size+3);

% h_legend=legend(sprintf('%s n=%d',Legend,n), 'Location', 'SouthEast');
% set(h_legend, 'FontSize',size);
xlabel('time (s)');
ylabel('z-score');
hold on

line([0,0],[y_axis1 y_axis2],'Color','k','LineWidth',1,'LineStyle','--');
% savefig(sprintf('%1$s %2$s', Legend,'.fig'))
% saveas(gcf,sprintf('%1$s %2$s', Legend,'.jpg'))
% saveas(gcf,sprintf('%1$s %2$s', Legend,'.pdf'))
