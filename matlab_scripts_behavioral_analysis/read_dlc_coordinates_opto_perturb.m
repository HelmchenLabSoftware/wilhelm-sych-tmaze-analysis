...
path_to_data = 'D:\Documents\data\wm_t_maze\opto_inhibition\video\optoLeftCorrect';
%path_to_performance = 'D:\Documents\data\wm_t_maze\opto_activation\performance';
...
median_trials_size = floor(median(trials_size,1));

% last 3 behavioral variables should be one number
median_trials_size = [median_trials_size(1:6) 1 1 1];
...
backup_mdl_speed_tj = [];
backup_mdl_angle_tj = [];
backup_mdl_speed_start = [];
backup_mdl_angle_start = [];
...
for count_trials = 1:size(b_mdl,2)
    ...
    if and( ~isempty(temp.speed.towards_tj), ~isempty(temp.speed.towards_start))
    ...
    input10 = sum(abs(temp.direction_of_turn.start)) + sum(abs(temp.direction_of_turn.tj));

    elseif isempty(temp.speed.towards_tj)
    ...
    elseif isempty(temp.speed.towards_start)

    input1 = resample(temp.position_y', median_trials_size(1), numel(temp.position_y));
    input2 = resample(temp.speed.full_trial', median_trials_size(2), numel(temp.speed.full_trial));
    input3 = median(backup_mdl_speed_start);
    input4 = resample(temp.speed.towards_tj', median_trials_size(4), numel(temp.speed.towards_tj));
    input5 = median(backup_mdl_angle_start);
    input6 = resample(temp.angle.towards_tj', median_trials_size(6), numel(temp.angle.towards_tj));
    input7 = mean(temp.direction_of_turn.start);
    input7(isnan(input7)) = 0;
    input8 = mean(temp.direction_of_turn.tj);
    ...
    temp_measures=[input1 input2 input3 input4 input5 input6 input7 input8 input9 input10];
    % use measures for reduced behavioral vector - no temporal information
    measures=[mean(input1) mean(input2) mean(input3) mean(input4) mean(input5) mean(input6) input7 input8 input9 input10];
    ...
    backup_mdl_speed_start = cat(1,backup_mdl_speed_start,input3);
    backup_mdl_angle_start = cat(1,backup_mdl_angle_start,input5);
...
temp_size = [median_trials_size(1:6) 1 1 1 1];
...
save('D:\Documents\data\wm_t_maze\opto_inhibition\b_mdl_results\optoleftCorrect.mat')

%% ----- check how Performance correlates across clusters -----

cd(path_to_performance)
% Get a list of all files and folders in the mouse folder
all_cafiles = dir('*.mat');
count_all_trials = 1;
count_ind = 1;
...
%         cadata.trial_ca(count_all_trials) = median(temp_data{1, count_trials},2,'omitnan');  
%         
...
% save workspace to compare left and right trials
%save('D:\Documents\data\wm_t_maze\photometry\ca_data\leftCorrect.mat')

%% ----- check how Ca dynamics correlates with behavioral variables -----
...
subplot(1,9,count_behav)

% input_behaviors for all trials
input_behaviors = median(input_to_plot{1,count_behav}.all,2,'omitnan');
input_ca_to_plot = input_ca.all;

% input_behaviors for cluster 1
% input_behaviors = median(input_to_plot{1,count_behav}.c3,2,'omitnan');
% input_ca_to_plot = input_ca.c3;

scatter(input_behaviors,input_ca_to_plot);
hold on

mdl = fitlm(input_behaviors,input_ca_to_plot);
plot(mdl)
hold off

title([Names_of_behaviors{1,count_behav} ' R^2= ' num2str(mdl.Rsquared.Adjusted)])
end